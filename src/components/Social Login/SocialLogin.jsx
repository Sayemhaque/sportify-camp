...
const SocialLogin = () => {
  ...
  const handleLoginGoogle = async () => {
    ...
    const user = { name: result.user?.displayName, image: result.user?.photoURL, email: result.user?.email, role: "student" }
  ...
  return (
    <div>
      <button onClick={handleLoginGoogle} className="btn-block flex items-center justify-center gap-3 bg-gray-300 font-bold py-2 rounded-full">
